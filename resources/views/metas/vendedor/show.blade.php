...
?><div class="row">
		 ...
		 <div clas ="row">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><h4 align="center">Unidades Establecidas en Meta</h4>
                  ...
                  <table id="desglose" class="table table-striped table-bordered table-condensed table-hover">
					 ...
					 ?> <?php $aux=0; ?>
                        <tr >
                          <td><?php if($meta->estatus==0){?><a href="" data-target="#modal-ajuste-{{$re->idarticulo}}" data-toggle="modal">{{$re->codigo}}</a><?php }else { echo $re->codigo; } ?> </td>
						      <td>{{$re->nombre}}</td> 
		...
		echo "<i class='fa fa-fw fa-thumbs-down'></i>-".$met; }?>
						...
						@include('metas.vendedor.modalajuste')
                        @endforeach
                        <tfoot>                    
