...
@include('almacen.articulo.empresa')
	...
	<a href="/ventas/indeximportar"> <button class="btn btn-success btn-xs">Importar</button></h3></a></div>
	</div>
...
$newdate=date("d-m-Y",strtotime($ven->fecha_hora));
    ?>
				<tr>
					...
					<td><small>{{$ven->tipo_comprobante}}-{{ $ven->serie_comprobante.'-'.$ven->num_comprobante}}<?php if ($ven->forma==1){ echo " *";} ?> </small></td>
                 ...
                 <?php if (($ven->forma==1)and($ven->formato==0)){?>   <li><a href="/ventas/formalibre/{{$ven->idventa}}_0">Forma Libre</a></li> <?php }  ?>
                  ...
                  <?php if ( $ven->devolu == 0){?><a href="{{URL::action('ReportesController@show',$ven->idventa)}}"   ><button class="btn btn-danger btn-xs">Devolucion</button></a><?php } else {?><button class="btn btn-warning btn-xs">Devuelta</button><?php } ?>
					</td>
				</tr>
