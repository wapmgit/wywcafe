                      ...
                      <tbody>
                        ...
                        <tr ><?php $cntline++; ?>
                          ...
                          <td>{{$det->articulo}}<small><?php if($det->iva>0){
							$costo=($det->precio_venta/(($det->iva/100)+1));
				$costo = number_format(($costo),2);	
				$var=(($det->precio_venta/(($det->iva/100)+1))*$det->cantidad);
				 $var = number_format(($var),2);
						  $acumbi=$acumbi+$var;
						  $acumiva=($acumiva+($costo*$det->cantidad*($det->iva/100)));
						 $acumiva = number_format(($acumiva),2);
                          ...
                          <td><?php $acumsub=$acumsub+((($det->cantidad*$costo)-$det->descuento)*$venta->tasa); echo number_format( ((($det->cantidad*$costo)-$det->descuento)*$venta->tasa), 2,',','.'); ?></td>
                        </tr>
                        @endforeach
						<?php for($i=$cntline;$i<10;$i++){ echo "<tr><td>&nbsp;</td></tr>"; }?>
                      </tbody> 
					  <tfoot>   
<tr><td colspan="4" align="center"></td><td><b>Subtotal Bs: </b></td><td><b><font size="3"  align="center"><?php echo number_format($acumsub, 2,',','.'); ?> </b></td></tr>					  				  
					</tfoot>
                  </table>
				  <table width="100%"><tr>
...
$(document).ready(function(){
	...
	window.location="/ventas/venta"; 
		...
		document.body.innerHTML =printcontenido;
	  	window.print();
		  window.location="/ventas/formalibre/<?php echo $venta->idventa."_0"; ?>";
