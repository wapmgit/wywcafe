...
class VendedoresController extends Controller
{
    ...
    public function index(Request $request)
    {
	 ...
	 if ($nivel=="A")
	 {
			 ...
			 $rol=DB::table('roles')-> select('newvendedor','editvendedor','actvendedor')->where('iduser','=',$request->user()->id)->first();
    ...
    {
			$clientes=DB::table('clientes')->where('vendedor','=',$id)->where('status','=',"A")
				->get();
...
//dd($notasnd);
	...
	{
		//dd($request->tipo);
        $paciente=Vendedores::findOrFail($request->id);
		if($request->tipo==0){
        $paciente->estatus=0;}
		else{ $paciente->estatus=1;}
        $paciente->update();
        return Redirect::to('vendedor/vendedor');
	}
