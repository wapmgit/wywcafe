 ...
 public function index(Request $request)
    {
        ...
        if ($request)
        {
            ...
            -> select ('i.idingreso','i.emision as fecha_hora','p.nombre','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.impuesto','i.estado','i.estatus',DB::raw('sum(di.subtotal)as total'))
            ->where('i.idempresa','=',$ide)
			-> where ('p.nombre','LIKE','%'.$query.'%')
            -> orderBy('i.idingreso','desc')
            -> groupBy('i.idingreso','i.fecha_hora','p.nombre','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.isaea','i.impuesto','i.estado')
                ->paginate(50);
     
     return view ('compras.ingreso.index',["rol"=>$rol,"ingresos"=>$ingresos,"searchText"=>$query]);
        }
    } 
	...
	//dd($monedas);
	   ...
	   $categorias=DB::table('categoria')-> where('idempresa','=',$ide)->where('condicion','=','1')->get();
        ...
        -> where('estatus','=','A')-> where('idempresa','=',$ide)->get();
...
public function show(Request $request, $id){
   ...
   $ingreso=DB::table('ingreso as i')
            -> join ('proveedor as p','i.idproveedor','=','p.idproveedor')
            -> select ('i.idingreso','i.fecha_hora','i.total','i.isaea','p.nombre','p.telefono','rif','direccion','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.impuesto','i.estado','i.base','i.miva','i.exento','i.estatus')
            ...
            ->get();
$recibonc=DB::table('mov_notasp as mov')-> where ('mov.iddoc','=',$id)-> where ('mov.tipodoc','=', $ingreso->tipo_comprobante)
            ->get();
            return view("compras.ingreso.show",["recibonc"=>$recibonc,"ingreso"=>$ingreso,"empresa"=>$empresa,"detalles"=>$detalles,"pago"=>$pago]);
}
public function notas($id){
   ...
   $ingreso=DB::table('ingreso as i')
            -> join ('proveedor as p','i.idproveedor','=','p.idproveedor')
            -> select ('i.idingreso','i.emision','i.fecha_hora','i.total','p.nombre','p.telefono','rif','direccion','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.impuesto','i.estado','i.base','i.miva','i.exento','i.estatus')
            ->where ('i.idingreso','=',$id)
            -> first();

            $detalles=DB::table('detalle_ingreso as d')
            -> join('articulo as a','d.idarticulo','=','a.idarticulo')
            -> select('a.nombre as articulo','d.cantidad','d.precio_compra','d.iddetalle_ingreso as iddetalle','a.iva','d.precio_venta','d.subtotal')
            -> where ('d.idingreso','=',$id)
            ->get();
