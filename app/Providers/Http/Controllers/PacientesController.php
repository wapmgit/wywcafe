	...
	public function edit($historia)
	{
		...
		$parroquias=DB::table('parroquias')->get();
		 $datos=DB::table('clientes as c')
			...
			->first();
			//dd($paciente);
		return view("pacientes.paciente.edit",["parroquias"=>$parroquias,"municipios"=>$municipios,"paciente"=>$paciente,"vendedores"=>$vendedor,"datos"=>$datos]);
	}
