...
return Redirect::to('ventas/venta');
    ...
    {   
         $empresa=DB::table('empresa')-> where('idempresa','=','1')->first();
            $lista=DB::table('articulo')->where('articulo.stock','>',0)
    ...
    public function ventasarticulo(Request $request)
    {
		...
		//	dd($datos);
			$query2=date("Y-m-d",strtotime($query2."- 1 days"));
			return view('reportes.ventasarticulo.index',["vendedor"=>$vendedor,"datos"=>$datos,"empresa"=>$empresa,"searchText"=>$query,"searchText2"=>$query2]);
	 ...
	 public function resumen()
    {
           $empresa=DB::table('empresa')-> where('idempresa','=','1')->first();
            ...
            ->join ('proveedor as p', 'c.idproveedor','=','p.idproveedor')
			->select ('c.idingreso','c.base','c.serie_comprobante','c.estado','c.total','c.saldo','c.fecha_hora','p.nombre','p.rif','p.telefono')
          ->where('c.saldo','>',0)
		    ->get();
		  $gastos=DB::table('gasto as g')
            ->join ('proveedor as p', 'g.idpersona','=','p.idproveedor')
			->select ('g.idgasto','g.saldo','g.monto','g.documento','g.fecha','p.nombre','p.rif','p.telefono')
          ->where('g.saldo','>',0)
          ->where('g.estatus','=',0)
            ->groupby('g.idgasto')
            ->get();
			...
			->select ('v.tipo_comprobante','v.num_comprobante','v.total_venta','v.saldo','c.nombre','c.telefono','c.cedula')
          ->where('v.saldo','>',0)
		  ->where('v.tipo_comprobante','=','FAC')
			...
			->first();   
        return view('reportes.resumen.index',["valor"=>$valor,"ventas"=>$ventas,"compras"=>$compras,"gastos"=>$gastos,"empresa"=>$empresa]);    
